...
- 특징
  ...
  - 여러 인스턴스에 한번에 마운트 가능
    - io/io2군만 같은 AZ에서 가능
    - 유스케이스
      - 매우 높은 application availability가 필요한 경우
      - 애플리케이션 레벨에서 race condition관련 처리가 따로 필요함
    - 한번에 16개의 EC2 인스턴스만 부착 가능
    - cluster-aware한 파일 시스템을 사용해야 함
      - xfs(x)
      - ex4(x)
  - 같은 AZ에서만 사용 가능
  ...
  - 암호화 기능
    - 암호화 대상
      - 볼륨에 저장된 데이터
      - 인스턴스와 볼륨 사이에 전송되는 데이터
      - 모든 스냅샷
      - 그 스냅샷으로 부터 생성되는 볼륨
    - 특징
      - 암호화 복호화는 투명하게(transparently) 다뤄짐
        - 즉, 나 자신을 할 일이 없음
      - 암호화는 아주 약간의 latency를 유발(대부분 무시가능)
      - KMS에서 키를 활용함(AES-256)
      - 암호화 되지 않은 스냅샷을 복사할때에 암호화 할 수 있음
    - 암호화 방법
      - 볼륨의 EBS 스냅샷을 생성
      - 해당 스냅샷을 암호화 함(copy)
      - 해당 스냅샷으로 부터 새 EBS볼륨을 생성
      - 생성된 EBS볼륨을 기존 인스턴스에 부착
- 기능
