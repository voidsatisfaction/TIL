...
初日のガイダンスとオンライン教育システムで勉強することができた。

### 質問する前には、きちんと自分の考えをまとめてから相手に聞きに行く

非常に大事である。内容としては、もちろん自分がわからないから聞きに行くことであろうが、ある程度自分がきちんと考え、言語化（紙に書くとか）してから聞くことが勉強になるし、相手が教えやすくする。

## 技術

### タイピングシステム

いつも、タイプがあることが正しいわけではない。ユーザーに近ければ近いほど、タイプがあること自体がよくなくなる可能性がある。例えば、フロントエンドでは
...
1. サーバーのテストを書くときに、サーバが他のサーバーのAPIを利用していて、そこのモックを`新しいサーバーではない`形で作る方法はあるだろうか。もともとは`http roundTripper`を使ったものにしようとしたが、それだと、テスト内のすべてのhttp requestがlistenされてしまう。
