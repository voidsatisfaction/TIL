...
- 인터럽트가 발생하여 그것을 처리하는 동안 또 인터럽트가 발생하면 어떻게 되는가?
  - Interrupt Enable bit 레지스터를 0으로 설정하여 처리중에 발생한 다른 인터럽트 무시
...
- 가상 메모리
  ...
  - 페이지 정보 표현
    ...
    - 몇 번째 페이지인지 나타내기
      - 프레임과 같음
    - 예시
      - `0000 0000 0000 0000 0000 0001 0000 0000 0011`
        - `0000 0000 0000 0000 0000 0001`는 페이지 번호 = 1
...
- RTOS
  ...
  - 다단계 인터럽트 방식 채용
    - 개요
      - 인터럽트가 발생하면 task목록 버퍼에다가 task삽입
        - task는 우선순위가 존재
      - ISR에서는 그냥 바로 반환
  - 다음 조건을 만족
    - 선점형 방식
...
- 리눅스, Windows는 RTOS인가?
  - 선점형 방식 x
    - OS의 시스템 함수(커널 코드)가 선점성을 지원하지 못함
      - 그 이유는 공유자원에 대한 문제(CreateFile을 하는 동안, DeleteFile로 문맥전환 되면...)
  - 우선순위 스케줄링 지원 o
  - 인터럽트 지연 o
