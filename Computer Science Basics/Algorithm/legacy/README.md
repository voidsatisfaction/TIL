...
- 세상을 분석하는 틀
  - Mathematical model vs Computational(Algorithmic) model
  - 수학적 모델이 자연 현상을 설명하려고 함
    - 가설 검증이 힘듬
  - 20세기에 들어서 Computational model을 이용해서 자연 현상을 simulation해서 가설검증을 함
...
- 그 데이터를 구현하는 "제작자"의 입장에서 테이터 타입의 값과 행위를 정의한 것

### 추상적 자료형(Abstract Data Type)의 정의

- 알고리즘이 문제를 해결하는데 필요한 자료의 형태와 연산들을 수학적으로 정의한 모델
- 그 데이터를 사용하는 "사용자"의 입장에서 데이터 타입의 **값** 과 **행위** 를 정의한 것
- 시간 복잡도와 공간 **복잡도** 역시 포함되는 경우도
