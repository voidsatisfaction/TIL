...
존재한다의 증명은, 직접 그 대상을 보여줄 수 있다. 혹은 그러한 대상이 무조건 존재해야함을 보여줄 수 있다.
...
- 함수
  - 개요
    ...
    - **함수식이 같아도 정의역이나 공역이 다르면 다른 함수다**
      - 왜냐하면, 함수는 관계이기 때문에 곱집합의 부분집합이기 때문
      - `f(x)=x^2, Dom(f)=R`
      - `g(x)=x^2, Dom(g)=C`
      - `f ≠ g`
...
- 함수가 같다
  - `f : X -> Y`와 `g : X -> Y`가 같은 함수 <=> `∀x∈X, f(x)=g(x)`
    - 함수를 관계의 일부로 생각했을 때, 결국 곱집합의 부분집합이므로 직관적으로 당연하다.
...
- 함수 `f : X -> Y`
  ...
  - 전단사
    ...
    - **일대일 대응**
    - 역함수의 유일성
      - 두 함수 `f : X -> Y`와 `g : Y -> X`가 `g・f=Ix`이고 `f・g=Iy`이면 f는 전단사함수이고 `g=f^-1`이다.
...
**각종 함수들은 함수적 정의와 관계적 정의로 정의될 수 있다**

함수적 정의는 정의역과, 공역과, 함수식으로 이루어진 정의

관계적 정의는 정의역과, 공역과, 치역으로 이루어진 정의

- 고등학교 교육과정 내
  - 항등함수
    - 함수적 정의
      - `Ix: X -> X`가 `∀x∈X, Ix(x) = x`를 만족
    - 관계적 정의
      - `{(x1,x2)∈X x X | x1 = x2}`
  - 상수함수
  - 역함수
  ...
  - 합성함수
    - 함수적 정의
      - 두 함수 `f : X -> Y`, `g : Y -> Z`에서 `∀x∈X, (g・f)(x) = g(f(x))`
    - 관계적 정의
      - `g・f = {(x,z)∈XxZ | ∃y∈Y, (x,y)∈f ∧ (y,z)∈g}`
      - 합성함수도 결국에는 관계의 합성이므로 그 성질은 관계의 합성의 성질을 따름
    - 함수적 정의와 관계적 정의가 같음을 증명하고, `g・f ⊆ X x Z`가 함수임을 보여라
    - 아래의 합성함수의 성질을 잘 공부해두자
    - f의 제한
      - `f : X -> Y`의 정의역 X의 부분집합 A가 주어졌을 때
      - `f・포함함수A : A -> Y`
        - 함수 f의 정의역을 기존 정의역인 X의 부분집합으로 제한시킴
  - 합성함수의 성질
...
- 고등학교 교육과정 외
  ...
  - 포함함수
    - 함수적 정의
      - `∀x∈A, i : A -> X`가 `i(x) = x (∈ A)`
        - 여기서 왜 일부러 `A -> X`인 이유는, 함수의 합성을 이용한 제한을 정의하고 싶기 때문
    - 관계적 정의
      - `{(a,a)∈AxX | a∈A}`
    - 항등함수의 축소함수
...
- 함수 `f : X -> Y`에서 `A ⊆ X`이고, `B ⊆ Y`일 때 다음이 성립한다
  ...
  - 상과 역상의 일반적인 성질
    - `f^-1(f(A)) ⊇ A`
      - `f^-1(f(A)) ≡ A`일 필요충분 조건은 f가 단사
    - `f(f^-1(B)) ⊆ B`
      - `f(f^-1(B)) ≡ B`일 필요충분 조건은 f가 전사
  - 상과 역상은 정의역의 원소가 집합일 때만 성립하는게 아니고, 단순히 값일 때에도 같은 이름으로 불림
