...
- 본질
  - 기술적 인프라(검색)이 도메인 로직에 포함되지 않도록 한다
    - c.f) 스프링
  - **검색(전역적 + 기술적인 방법)으로 가져오는 것은 어그리거트여야만 한다(응집력)**
    - 나머지는 연관관계의 탐색으로 가져온다
    - *그 어그리거트는 좋은대로 필터링 해도 상관 없다*
      - 어차피 전역적 서칭이므로?
  - 데이터베이스 질의 / 메타데이터 매핑 장치 캡슐화
- 배경
  - 데이터베이스의 검색 vs 연관관계
    - 데이터베이스의 검색
      - 어디서든 이용가능 & 곧바로 어떠한 객체에도 접근 가능(전역적)
    - 연관관계(탐색)
      - 응집성
...
- 리포지토리
  - 개요
    ...
    - 생명주기 초기에서 마지막 단계에 이르기까지 어그리거트의 루트에 대한 접근을 제공하는 각종 응집력 있는 책임이 포함
    - 실질적으로 직접 접근해야 하는 어그리거트의 루트에 대해서만 리포지토리를 제공하고, 모든 객체 저장과 접근은 리포지토리에 위임해서 클라이언트가 모델에 집중하게 하기
  - 특징
    - 데이터베이스 질의 / 영속화 및 메타데이터 매핑에 대한 장치를 캡슐화
    - 클라이언트에서 요구하는 기준에 근거해 객체를 선택하는 다양한 질의 구현 가능
    - 인스턴스의 개수와 같은 요약 정보도 반환 가능
    - 특정 수치 속성 객체의 합계와 같은 요약 계산 반환 가능
  - 이점
    - 영속화된 객체를 획득하고 해당 객체의 생명주기를 관리하기 위한 단순한 모델을 클라이언트에 제시
    - 영속화 기술과 다수의 데이터베이스 전략, 다수의 데이터 소스로부터 애플리케이션과 도메인 셜계를 분리
    - 객체 접근에 관한 설계 결정을 전해줌
      - 객체 접근은 알아서 해라
    - *테스트에서 상요할 가짜 구현을 손쉽게 대체할 수 있음(?)*
- 리포지토리에 질의하기
  - 개요
    - 질의에 구체적인 매개변수를 직접 입력하는 것
    - *명세 기반 질의(?)*
      - 질의의 획득 방법에는 신경 쓰지 않고, 원하는 바를 서술 가능
      - 직접 입력도 가능하게 해야 함
- 리포지토리 구현
  - 개요
    - 어떤 DB, 메모리, 파일 등 어떤 방법이든 다 캡슐화 해서 클라이언트 코드를 동일하게 유지
  - 중요 사항
    - 타입을 추상화한다
      - 타입은 계층구조상 추상 상위 클래스가 될 수 있음
      - 인터페이스가 될 수도 있음
    - 클라이언트와의 분리를 활용한다
      - 영속화 로직을 자유롭게 변경하고, 캐싱도 추가 가능
    - 트랜잭션 제어를 클라이언트에 둔다
      - 클라이언트에서 트랜잭션의 컨텍스트를 둠
- 프레임 워크 활용
  - 되도록 설계 형식과 조화되는 프레임워크나 그런 요소 활용
  - 아닌경우, 억지로 맞추지 말고, 프레임워크 명세를 준수
- 팩토리 vs 리포지토리
  - 둘을 명확히 분리하자
  - 팩토리
    - 새로운 객체 생성
    - 영속화 책임은 제외
    - 역할은 복잡한 객체를 인스턴스화 하는 것
    - find or create 자제하기
      - VO는 그저 팩토리에서 생성하면 됨
      - 새로운 객체와 이미 존재하는 객체를 구분하는 것은 도메인에서 중요하다
  - 리포지토리
    - 기존 객체 찾아내기
    - 객체의 재구성
    - 객체 생명주기상의 중간단계
- 관계형 데이터베이스를 위한 객체 설계
  - 개요
    - 기술적으로 관계형 테이블 설계에서 도메인 모델을 반영할 필요는 없다
      - 매핑 도구가 객체 모델과 관계형 모델 간의 중대한 차이를 서로 이어줄 만큼 충분히 정교함
