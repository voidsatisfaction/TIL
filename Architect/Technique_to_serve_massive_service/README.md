...
- Ch7. 알고리즘 실용화
  - 알고리즘과 평가
- Ch11. 대규모 데이터 처리를 지탱하는 서버/인프라 입문 - 웹 서비스의 백엔드
- Ch12. 확장성 확보에 필요한 사고방식 - 규모 증대와 시스템 확장
- Ch13. 다중성 확보, 시스템 안정화 - 100%에 근접한 가동률을 실현하는 원리
- Ch14. 효율향상 전략 - 하드웨어의 리소스 사용률 높이기
- Ch15. 웹 서비스와 네트워크 - 서비스의 성장
...
- OS캐시
  - *OS캐시를 활용할경우, 캐시가 더이상 disk의 최신 상황을 반영하지 못할경우, 어떻게 그것을 파악하며 캐시를 업데이트 해주는가?*
    ...
    - 애초에 디스크에 write나 update를 하는경우에 캐시 데이터를 먼저 갱신하주는 것인가?
      - MySQL의 InnoDB의 버퍼풀 같은 경우에는 그렇게 하는 듯
- Index
  - *B트리, B+트리 구조 이해*
- Load
  - 개요
...
- Linux의 페이지(디스크) 캐시 원리
  - 과정
    ...
    - 2 읽어낸 블록을 메모리에 씀
      - 해당 블록은 메모리상에 위치시켜야 함
        - 프로세스는 디스크에 직접 액세스 불가
    - 3 OS는 그 메모리 주소를 프로세스에 논리적 주소로 알려줌
    - 4 프로세스가 해당 메모리에 액세스
    - 5 데이터 읽기가 마쳐도 메모리 블록을 해제하지 않고 남겨둠
    - 6 다른 프로세스가 같은 디스크에 접근할 때, 남겨둔 페이지 재사용
      - 페이지 캐시
  - 특징
...
- 핵심
  - 데이터 크기를 줄임
    - 압축
  - 메모리를 늘림(메모리를 늘린다는 것은, 페이지 캐시를 사용하겠다는 뜻)
    - Scale up
    - Scale out
      - 국소성
- 기본 자세
...
- 핵심
  - 문제 해결에 적합한 알고리즘과 데이터 구조를 사용
  - 대상이 되는 데이터가 크면 클수록 선택에 따른 차이가 현저해짐
...
- 알고리즘
  - 개요
    - 값을 입력하면 값을 출력하는 정의된 계산 절차
  - 의의
    - 문제를 유한한 자원으로 해결하기 위함
    - 디자인 패턴과 마찬가지로 엔지니어의 공통언어
    - 새로운 문제에도 대처 가능
  - 계산량과 상수항
    - Order표기는 알고리즘을 비교할 때 편리하지만, 구현을 포함해서 생각할때 그게 전부는 아님
      - 측정이 중요함
    - e.g)
      - B+트리(디스크 고려),
      - 퀵정렬(CPU캐시 고려)
- 데이터 구조
  - 개요
    - 알고리즘에서 자주 사용하는 조작에 맞춰 데이터 구조를 선택해야 함
- 데이터 압축과 속도
  - 개요
    - CPU가 여유 있고, I/O가 바쁜 경우에는 압축을 활용해서 I/O 부하를 줄이고, CPU 부하를 늘리자

## Ch11. 대규모 데이터 처리를 지탱하는 서버/인프라 입문 - 웹 서비스의 백엔드

### 웹 서비스의 인프라

신뢰성 100%보다는 다른 것을 중시하자

- 세가지 포인트
  - 저비용 고효율
  - 확장성이나 응답성에 대한 설계 중시
  - 서비스 사양의 변화에 유연하게 대응할 수 있는 인프라
- 하테나의 규모
  - 150만 유저수
  - 수십억 액세스/월
  - 피크 시 회선 트래픽량 850Mbps
  - 하드웨어 600대 이상(가상화로 1300대)

하테나 인프라 아키텍처

![](./images/hatena_infrastructure_architecture1.jpg)

- 하테나의 인프라 아키텍처
  - 리버스 프록시 - API 서버 - DB 서버
...
- 서버의 성능
  - 4core CPU, 8GB 메모리
    - 수천 요청/분(대략적)
    - 100만 PV/월
  - 4core CPU 2개, 32GB 메모리
    - 수천~1만 건/분
    - 200만 PV/월
- 하테나의 문제
  - 수억 PV/월 정도의 트래픽을 어떻게 확장시켜 나갈것인가
    - 서버 1대로는 동작할 수 없음
- 계층별 확장성
  - API 서버
    - 구성이 동일하고 상태를 갖지 않음 -> 용이
  - 데이터 소스(DB, 파일 서버 등)
    - read의 분산 -> 비교적 용이
      - 메모리를 많이 탑재, 기타
    - write의 분산 -> 어렵다
...
- 부하 파악을 위한 지표
  - **Load Average**
    - 정의
      - 프로세스가 언제든지 동작할 수 있는 상태이지만, 아직 실제 CPU가 할당되지 않아서 대기상태에 있는 프로세스 수의 평균치 + I/O를 대기하는 프로세스의 수의 평균치
...
- SPoF(Single Point of Failure)를 제거함으로써, 가동률을 높이고 있음

### 다중성 확보

- 문제
  - 수십 대의 서버가 있는 경우, 1,2 대가 멈추는 일은 일상다반사
    - 원인
      - 인위적인 실수
      - 물리적으로 고장남
      - 메모리 이상 등
- 대응
  - 로드벨런서
    - 헬스 체크(API, DB 서버 살아있는지)
    - 다운된 쪽은 트래픽을 보내지 않고, 나머지로 트래픽 처리
      - 페일 오버
- **다중성 확보**
  - **API 서버**
    - stateless하므로, 수평 확장
  - **DB 서버**
    - 마스터
      - 멀티 마스터
        - 서로가 서로를 replication
        - 약간의 지연은 받아들이고 다중성을 확보
        - VRRP 프로토콜로 감시해서, 한쪽이 분리된 것을 알면 Active 마스터로 승격
    - 슬레이브
      - 서버 여러 대 나열
  - **스토리지 서버**
    - 개요
      - 이미지 파일과 같은 미디어 파일을 저장하기 위한 분산 스토리지 서버
      - 너무 노드당 disk 용량이 높아져도, 디스크 읽기/쓰기 I/O 성능이 병목이 되어서 100% 용량을 다 사용할 수가 없게 되는 경우가 있음
- 시스템 안정화
  - 방식
    - **적절한 버퍼 유지**
      - CPU, 메모리는 7할까지만 사용하도록 한다
        - 남는 공간을 두어서 막혀서 장애가 되지 않도록 함
        - 7할 넘으면, 서버를 추가하거나 메모리르 늘림
    - **불안정 요인 제거**
      - SQL 부하대책
        - 높은 부하의 SQL은 격리시킨 배치용 DB를 준비해서 그곳에서 작동
      - 메모리 누수 줄이기
        - 엔지니어가 신경써야 함
      - 비정상 동작 시 자율제어(본질적인 해결책은 아님)
        - 자동 DoS 판정
          - IP기반 엑세스 차단
        - 자동 재시작
          - 메모리 누수가 너무 심해서 스왑이 발생하는 경우 재시작을 하도록 함
        - 자동 쿼리제거
          - DB 서버에 10초에 한번씩 어떤 쿼리가 실행되고 있는지 파악해서 소요 시간이 긴SQL은 kill
  - 요인
    - API/서비스 레벨
      - 기능 추가
      - 메모리 누수
      - 지뢰
        - 특정 URL에 리퀘스트 보내면 아무리 시간이 지나도 응답이 오지 않음
        - e.g) 하테나 다이어리의 코멘트에서 1만 건 정도의 코멘트가 달린 엔트리
          - DateTime 모듈을 사용한 오브젝트 생성 -> 메모리를 크게 잡아먹음 -> 1만건의 코멘트는 수백 MB나 되는 메모리를 사용 -> 스왑 사용
      - 사용자의 액세스 패턴
        - 갑작스럽게 액세스가 증가하는 경우
        - 캐시 서버를 추가해서, 게스트 사용자의 경우는 캐시를 반환
      - 데이터량 증가
      - 외부연계 추가
        - 외부 서비스가 다운되는 경우 존재
          - 외부 노이즈에 견딜 수 있는 시스템 구현
    - 하드웨어
      - 메모리, HDD 장애
        - 하드웨어 장애는 일상적
        - 로드밸런서의 헬스체크를 하고, 이상이 발생했을 경우, 트래픽을 다른쪽으로 보냄
      - NIC 장애
...
- 하테나의 경우
  - 최신 Linux커널을 사용하면 30만패킷/초가 한계
  - 평균 패킷 길이가 300바이트 => 1Gbps
- 대책
  - PC라우터를 여러 대 병렬화
    - *애초에 엔트리포인트는 하나이지 않은가? 어떻게 문제를 해결한거지? 도메인별 라우터가 다른가?*
  - Cisco 라우터 구입
...
- ARP 테이블의 한계
  - 500건
- 호스트가 많은 서브넷은 브로드캐스팅 패킷이 서서히 증가해서 CPU가 잠식이 됨
...
- 대안
  - CDN(Content Delivery Network)
    - 개요
      - 지리적으로 분산된 프록시 서버와 데이터센터의 네트워크
        - 세계 각지에 서버를 두고 거기에 미디어를 캐싱시켜서 사용자가 가지러 갈 때 가장 가까운 서버로 액세스해서 미디어를 다운로드 하도록 하는 것
      - 글로벌 캐시 서버
    - 장점
      - 메인 서버의 트래픽 줄임
      - Bandwidth 코스트 줄임
      ...
      - DDoS 공격 회피
    - request route 원리
      - DNS가 동일 domain request에 대해서, source IP와 가장 가까운(최소 홉) CDN서버의 ip를 제공 or anycast(하나의 ip로 여러 서버를 나타내도록)
...
- 캐시 시스템
  ...
  - 리버스 프록시
    - 외부 클라이언트가 내부 서버에 액세스할 때 사이에 두는 프록시
    - 액세스 집중시에 방대한 요청을 API서버로 넘어가지 않도록 방지
  - 적어도 3대씩은 준비해서 다중화해두자
- 계산 클러스터
  - Hadoop
    - 하테나 다이어리의 액세스 로그는 하루에 4GB
      - 1개월분 120GB
      - 월간 고유 사용자를 계산하려면, 이 로그를 한번에 처리해야 함
      - 병렬처리가 가능한 계산 클러스터가 필요
