...
![](./images/ch1/definition_of_limit1.png)

- 까다로운 문제
  - prove that `lim_{x->3}(x^2) = 9`
- 주의
  - **모든 극한을 구하는 문제를 입실론-델타 논법을 사용해서 구할 필요가 없다. limit law들이 입실론 델타 논법을 이용해서 증명가능하기 때문에 임의의 함수들이 수렴하는 것을 알면 limit law로 쉽게 극한값을 구할 수 있다.**

![](./images/ch1/infinite_limit_graph1.png)

- `+` infinite limit
  - 정의
    - 함수 `f`가 실수 a를 포함하는 열린구간에서 정의되어 있을 떄(a자체는 정의되지 않아도 됨), `lim_{x->a}(f(x)) = ∞ <=> ∀M>0, ∃δ>0, 0<|x-a|<δ => f(x)>M`
- `-` infinite limit
  - 정의
    - 함수 `f`가 실수 a를 포함하는 열린구간에서 정의되어 있을 떄(a자체는 정의되지 않아도 됨), `lim_{x->a}(f(x)) = -∞ <=> ∀M<0, ∃δ>0, 0<|x-a|<δ => f(x)<M`
...
![](./images/ch1/continuity1.png)

- 연속의 정의
  - 함수 f는 a에서 연속 <=> `lim_{x->a}(f(x)) = f(a)` <=> 다음 세 조건을 만족
    - ① `f(a)`가 정의되어 있음
    - ② `lim_{x->a}(f(x))`가 존재함
    - ③ `lim_{x->a}(f(x)) = f(a)`
  - 함수 f는 a의 오른쪽으로부터 연속
  ...
  - 함수 f는 a의 왼쪽으로부터 연속
    - `lim_{x->a-}(f(x)) = f(a)`
  - 함수 f는 구간에서의 연속
    - 함수 f가 특정 구간의 모든 값에서 연속 <=> 함수 f는 특정 구간에서 연속
- 특징
  - (f가 a에서 연속일 경우) x가 a로 한없이 가까워 지면, f(x)가 f(a)로 한없이 가까워진다
  - (연속함수의 경우) x의 변화를 최대한 작게 하므로써 f(x)의 변화를 충분히 작게 할 수 있다
- 실생활 예시
  - 연속
    - 속도, 키
  - 불연속
...
- 복잡한 함수를 쉽게 연속인지 판단하기 위한 정리
  - `f, g`가 a에서 연속 ∧ c가 정수 => 다음 함수들이 a에서 연속
    - `f + g`
    - `f - g`
    - `cf`
    - `fg`
    - `f/g if g(a) != 0`
      - *limit law에서 `f/g`도 성립하는 이유? 증명?*
- 다항식과 연속의 관계
  - ① 임의의 다항식은 `R = (-∞, ∞)`에서 연속이다
  - ② 임의의 유리함수는 정의역의 범위에서 연속이다
