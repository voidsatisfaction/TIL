...
- *벡터공간의 정의가 체F에 대한 가군인데, 체의 집합은 원래 가군의 집합을 포함하는 것 아닌가? 체F에 대한 가군이라는게 무슨 뜻인지?*
  - 영상에서는 원소를 가져온다고 했는데 그게 무슨 뜻?
...
*여기서 노름 공간을 둔다는 것은, 벡터공간 V에 dot product 연산을 부여한다는 것(?)*
...
- 기저(basis)
  - 정의
    ...
    - `B ⊆ V, B는 선형독립 ∧ span(B)=V`를 만족하는 집합`B`
- 차원(dimension)
  - 정의
