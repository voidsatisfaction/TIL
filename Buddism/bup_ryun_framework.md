...
- 깨달음이란
  - **현실의 상황에 의거하여 일어난 자신의 마음상태가 모순적이라는 것을 파악하는 것**
    ...
    - 물론 input을 변화시켜도 함수의 결과값은 변하나, 함수 자체의 로직이 잘못되어있으면 다른 input이 들어왔을때 에러(괴로움)를 발생시킬 가능성이 크다. 반대로 함수 자체의 로직이 잘 되어있으면 어떠한 input이 와도 에러를 발생시킬 가능성이 낮다. 오히려 새로운 효과를 얻을 수도 있다.
