...
![](./images/batch_normalization2.gif)
...
## 6. 합성곱 신경망(CNN - convolutional neural network)
...
- 완전연결(fully-connected) 신경망
  - 완전연결
    - 인접하는 계층의 모든 뉴런과 결합되어 있는 것
  - 완전연결 계층을 Affine 계층 이라는 이름으로 구현
  - 구조
    - `(Affine -> ReLU) -> (Affine -> ReLU) -> ... `
  - 문제점
    - 데이터의 형상이 무시
      - 이미지: 가로・세로・채널(색상) 으로 구성된 3차원 데이터
      - 그러나 완전연결 신경망에서는 1차원 데이터로 flatten 해줌
        - 3차원 속에서 의미를 갖는 본질적인 패턴이 상쇄
- CNN
  - 개요
  - 구조
    - `(Conv -> ReLU -> Pooling) -> (Conv -> ReLU -> Pooling) -> (Conv -> ReLU) -> (Affine -> ReLU) -> (Affine -> ReLU) -> ...`
      - 출력에 가까운 층에서는 `Affine -> ReLU`계층 사용 가능
      - 출력 계층에서는 `Affine-Softmax`조합을 그대로 사용
  - 완전연결 신경망과의 차이점
    - 3차원 데이터로 입력 받고, 다음계층으로 3차원 데이터로 전달
      - 이미지처럼 형상을 가진 데이터를 제대로 이해할 가능성이 더 큼

### 6.2 합성곱 계층

![](./images/convolutional_operation1.jpeg)

*애초에 합성곱은 수학적으로 무엇을 의미하는 것인지?*
...
- 합성곱 연산
  - 이미지 처리에서 말하는 필터 연산에 해당
    - 입력 데이터에 필터(커널)를 적용
    - 데이터 형상을 `(높이, 너비)`로 표기(2차원의 경우)
  - 계산방법
    - 필터의 윈도우를 일정 간격으로 이동해가며 입력 데이터에 적용
    - FMA(Focused Multiply Add)
      - 입력과 필터에서 대응하는 원소끼리 곱한 후 그 총합을 구함
      - 그 결과를 출력의 해당 장소에 저장
      - 모든 장소에서 수행하면 합성곱 연산의 출력이 완성
- CNN
  - 합성곱 연산에서 필터의 매개변수
    - 가중치
  - 편향
    - 합성곱 연산의 편향
  - 패딩(P)
    - 합성곱 연산을 수행하기 전에 입력 데이터 주변을 특정 값(e.g 0)으로 채우는 것
    - 출력의 크기를 조정할 목적으로 사용
  - 스트라이드(S)
    - 필터를 적용하는 위치의 간격
      - 스트라이드를 2로하면 윈도우가 두 칸 씩 이동, 출력의 크기가 작아짐

합성곱 연산 크기 식

![](./images/convolutional_operation2.gif)

3차원의 합성곱 연산

![](./images/convolutional_operation3.jpeg)
