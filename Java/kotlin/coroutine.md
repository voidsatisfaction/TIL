...
import coroutine_test.utilities.StringUtils
...
suspend fun main() {
    ...
    // child coroutine의 job을 바꾸어 계층 구조를 끊었다
    coroutineScope {
        printThreadId(2)
...
- 구성
  ...
  - **Job**
    ...
    - parent-child 계층 구조
      ...
      - **job을 만약, `launch`나 `async`시에 메서드 인자로서 넘겨주면(e.g `launch(Job()) { ... }`), 해당 launch는 부모 job과는 독립적으로 됨**
    - c.f) SupervisorJob
