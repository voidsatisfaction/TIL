...
- 관점
  - 우리가 알고싶은 것은 리밸런싱 전과 리밸런싱 후의 '모집단'의 성공률의 차이가 통계적으로 유의미 한 것인지
  - 각 레벨에 대한 게임 try 사건의 결과는 성공이냐 실패냐 두가지의 결과가 나오고, 각각의 결과는 이전 플레이한 게임의 결과에 영향을 받지 않는다 -> 고 가정
    - 독립 베르누이 시행
  - 표본은 일정기간 동안, 리밸런싱 전 후에 해당 게임의 레벨을 즐겼던 사람들이고, 리밸런싱 후의 표본집단의 각 1회 시도당 성공, 실패를 1, 0값으로 두고, 그러한 각각의 게임 시행을 확률변수 X로 두면 X ~ Bi(1, p1) (여기서 p1은 리밸런싱 후의 모집단의 성공률) 이고 마찬가지로 리밸런싱 전의 게임 시행을 확률 변수 Y로 두면, Y ~ Bi(1, p2) (p2는 리밸런싱 후의 모집단의 성공률)
  ...
  - 여기서 확률변수 p^1과 p^2가 정규분포를 따르므로, 정규분포의 차의 재생성(reproductive property)에 의해서 확률변수 p^1 - p^2 역시 정규분포를 따름
...
- 생각 해봐야 할 점들
  - 개개인의 실력은 고려하지 않아도 되는가?
    - 전체를 뭉퉁그려서 전반적인 확률을 고려?
  - 세그먼트 별로 분석도 가능
    - 아이템을 쓴 후에 통과율과 쓰지 않고 통과율 등
- 생각
  - 각 레벨의 표본 성공률이 정규분포를 따르므로, 스테이지(15레벨 그룹)역시 정규분포의 합의 재생성을 이용해서, 유의미한 변화가 있었는지 비교 가능
  - 스테이지 뿐 아니라 모든 레벨을 뭉뚱 그려서 전체 게임의 성공률에 변화가 있었는지도 마찬가지로 정규분포의 재생성을 이용해서 알 수 있음
- 감상
