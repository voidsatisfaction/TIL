...
IP란, Internet Protocol의 약자이다. 정확히는 IP 주소라고 하는 것이 맞다.
...
- IP주소는 ICANN이라는 기관이 국가별로 사용할IP 대역을 관리
  - A클래스: 두번째 세번째 네번째 주소 부여 가능
  - B클래스: 세번째 네번째
  - C클래스: 네번째

#### 사설IP

- 사용자가 임의로 부여하고 사용할 수 있는 IP대역
- 인터넷상에서 서로 연결되지는 않음
  - A클래스: `10.0.0.0` ~ `10.255.255.255`
  - B클래스: `172.16.0.0` ~ `172.31.255.255`
  - C클래스: `192.168.0.0` ~ `192.168.255.255`
  - 즉, 위의 대역은 공인 IP가 없는 상태에서 아무 주소나 할당해도 문제 없다.
- 공유기에 공인 고정IP를 부여하고 회사나 가정 내의 IP 주소를 사설IP로 등록해서 인터넷에 접속하는 방식이 널리 사용됨
  - 단, 사설 IP를 부여한 컴퓨터는 공인 IP에서 알아볼 수가 없다.

#### 고정IP / 유동IP

- 고정IP: 컴퓨터에 고정적으로 부여된 IP로 한 번 부여되면 IP를 반납하기 전까지는 다른 장비에 부여할 수 없는 IP 주소
- 유동IP: 장비에 고정적으로 IP를 부여하지 않고 컴퓨터를 사용할 때 남아 있는 IP중에서 돌아가면서 부여하는 IP

- 인터넷 상에서 서버를 운영할 때는 공인 IP를 고정 IP로 부여해야 한다.
- 인터넷 서비스 업체는 가정마자 공인 IP를 유동 IP로부여, 공유기 내부는 사설 IP를 유동 IP로 부여
