...
- 정의
  - **라이프사이클이 끝난 외부 함수의 environment에 있는 변수를 참조할 수 있는 것**
- 특징
  - 외부 함수의 변수는 같은 변수(같은 메모리 주소를 공유) 인 경우, 내부함수에서 호출시점에 지속적으로 값을 트래킹 가능하다
- **c.f) react hook과 stale closure**
  - 그런데 react hook의 경우, useState에서 설정되는 변수는 항상 immutable하게 세팅된다(component로 전달되는 props도 immutable - props가 변경되면, 아예다른 메모리의 변수가 생김) 즉, set함수로 다른 값으로 세팅하면 메모리 주소가 바뀌는 아예 다른 변수가 된다.
    - 따라서 deps를 []로 두어서 초기에만 정의되는 함수의 경우, 초기 변수의 값만을 가지고 로직을 실행한다.
    - 그러므로 deps를 해당변수로 두거나, 아예 두지 않거나, 그냥 functional component내부의 함수로 정의해서 함수가 훅이 변경될때마다 매번 새로 정의되게 하면 stale closure문제를 해결할 수 있음
