...
- 실행
  - 응용 프로그램에서 `open(...)`함수를 실행하면 libc에서 제공한 `open`함수 속에서 인수 데이터를 레지스터에 넣고 소프트웨어 인터럽트 실행
  - 소프트웨어 인터럽트에 의해, 인터럽트 벡터 숫자를 이용해 ISR이 있는 커널의 인터럽트 처리 위치를 찾아 해당 주소로 실행을 옮김
    - CPU는 권한수준이 최고수준인 실행모드
...
- 개요
  - CPU가 프로그램을 실행하고 있을 때, 입출력 하드웨어나 소프트웨어 등의 장치에 예외상황이 발생하여 처리가 필요할 경우, CPU에게 알려 처리할 수 있도록 하는 것
...
- ISR 점프 방식
  - 일반적으로 하드웨어로부터 인터럽트 접수
  - 인터럽트 벡터(어떤 하드웨어에서 보낸 것인지) 파악
  - ISR 주소값 찾기
    - 정해진 주소값으로 무조건 점프
